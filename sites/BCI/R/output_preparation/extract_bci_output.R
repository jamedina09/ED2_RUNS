#!/usr/bin/env Rscript
# Extract BCI ED2 output at a chosen time resolution into tidy tables, saved
# as both CSV (external tools) and RDS (plot_bci_output.R). One script for
# every resolution; one flag controls which.
#
# Usage: Rscript sites/BCI/R/output_preparation/extract_bci_output.R --exp=<id> [--resolution=monthly|daily|hourly|yearly]
...
# or --output_freq=monthly,daily,hourly,yearly; see build_bci_ed2in.R)
#
# --- Two structurally different kinds of output, confirmed by direct HDF5
#     inspection, not assumed -----------------------------------------------
#
# monthly/daily/hourly (--resolution=monthly|daily|hourly) are ED2's
# flux/state time-series output: carbon fluxes (GPP, NPP, respiration),
# water/energy fluxes, and PFT x DBH-size-class stocks (biomass, LAI, stem
# density, storage carbon, basal area). Written to:
#   - timeseries_output_<resolution>.{csv,rds}  - ecosystem-scale (one row
#     per timestep)
#   - sizeclass_output_<resolution>.{csv,rds}   - PFT x size-class (one row
#     per timestep x pft x size-class)
#
# yearly (--resolution=yearly) is a COMPLETELY DIFFERENT kind of output: a
# demographic census (growth, mortality, recruitment, harvest by PFT and
# size class, plus ecosystem-wide totals of each), not a yearly-mean version
# of GPP/NPP/AGB_PY. Confirmed by direct inspection of a real
# analysis-Y-*.h5 file: it contains zero MMEAN_/DMEAN_/FMEAN_/YMEAN_-prefixed
# variables and no GPP/NPP/respiration/water/energy variables at all -
# instead TOTAL_AGB/TOTAL_AGB_GROWTH/TOTAL_AGB_MORT/TOTAL_AGB_RECRUIT (and
# the basal-area equivalents) at ecosystem scale, and
# AGB_GROWTH/AGB_MORT/AGB_CUT/BASAL_AREA_GROWTH/BASAL_AREA_MORT/
# BASAL_AREA_CUT as native (17 x 11) PFT x size-class arrays. Written to:
#   - demographic_output_yearly.{csv,rds}          - ecosystem-scale totals
#   - demographic_sizeclass_output_yearly.{csv,rds} - PFT x size-class rates
# (deliberately NOT named timeseries_output_yearly.*/sizeclass_output_yearly.*
# - those names are reserved for the flux/state variable set above, which
# yearly output does not contain at all, so using the same filenames would
# be misleading about what columns to expect.)
#
# --- Choosing which variables to extract (--variables, --sizeclass_variables) --
# Applies to both branches above, but each has its own default set and its
# own valid variable names (see ECOSYSTEM_VAR_MAP/SIZECLASS_RAW_NAME for
# monthly/daily/hourly, YEARLY_ECOSYSTEM_VAR_MAP/YEARLY_SIZECLASS_RAW_NAME
# for yearly, all defined below). By default, extraction pulls every
# variable in the relevant default set; pass a comma-separated list to
# extract a different subset (or, for --variables only, add a raw ED2
# variable name not in the curated set):
#
#   --variables=GPP,NPP,LeafTemp                (at monthly/daily/hourly)
#   --variables=AGB_total,AGB_growth             (at yearly)
#   --sizeclass_variables=AGB,NPLANT             (at monthly/daily/hourly)
#   --sizeclass_variables=AGB_growth,AGB_mort     (at yearly)
#
# --variables accepts either this script's own friendly column names or,
# for monthly/daily/hourly only, any raw ED2 variable name straight from
# variable_catalog.csv (see R-tools/describe_variables.R) for a _PY
# polygon-scale variable not in the curated set, with the resolution prefix
# optionally included or omitted (e.g. `ATM_TMP_PY` or `MMEAN_ATM_TMP_PY`
# both work at --resolution=monthly - this script strips a leading
# MMEAN_/DMEAN_/FMEAN_ and re-adds the correct one for whichever
# --resolution you asked for). At --resolution=yearly, --variables only
# recognizes YEARLY_ECOSYSTEM_VAR_MAP's own friendly names (no raw-name
# fallback), since yearly's raw variable names don't follow a per-resolution
# prefix convention. A requested variable that doesn't exist at all in the
# file becomes an all-NA column rather than an error - check
# variable_catalog.csv if a column comes back empty.
#
# --sizeclass_variables is a CLOSED list in both branches - at
# monthly/daily/hourly it must be a subset of LAI, AGB, NPLANT, Bstorage,
# BasalArea (native 17x11 arrays, any resolution) and GPP, NPP
# (cohort-aggregated, monthly-only - see the cohort-flux note below); at
# yearly it must be a subset of AGB_growth, AGB_mort, AGB_cut,
# BasalArea_growth, BasalArea_mort, BasalArea_cut - since these are the
# only PFT x size-class-shaped variables ED2 writes at each resolution.
#
# Run R-tools/describe_variables.R --site=BCI --exp=<id> first if you're not
# sure what raw variable names are available to add.
#
# --- How ED2's monthly/daily/hourly output resolution works (confirmed by
#     inspecting real output files, not assumed) ----------------------------
...
# build_bci_ed2in.R sets OUTFAST=-1 for hourly output); yearly =
# analysis-Y-*.h5 (never packed, one file per simulated year).
#
...
valid_res <- c("monthly", "daily", "hourly", "yearly")
...
file_tag <- c(monthly = "E", daily = "D", hourly = "I", yearly = "Y")[[resolution]]
...
# A file's (year, month, day) triplet - months/days are "00" placeholders
# (not real calendar values) for monthly and yearly files respectively,
# confirmed by direct inspection (analysis-E-YYYY-MM-00-*.h5,
# analysis-Y-YYYY-00-00-*.h5) - not a valid as.POSIXct() date, so anchor
# those fields to 1 instead, keeping the datetime column always valid.
parse_file_date <- function(fname) {
  m <- regmatches(fname, regexec("analysis-[A-Z]-(\\d{4})-(\\d{2})-(\\d{2})-(\\d{6})", fname))[[1]]
  year <- as.integer(m[2])
  month <- if (resolution %in% c("monthly", "yearly")) 1L else as.integer(m[3])
  day <- if (resolution %in% c("monthly", "daily", "yearly")) {
    if (resolution == "daily") as.integer(m[4]) else 1L
  } else {
    as.integer(m[4])
  }
...
if (resolution == "yearly") {
  # ===========================================================================
  # YEARLY (demographic census) extraction - see this script's header for why
  # this is a completely separate branch from monthly/daily/hourly below.
  # ===========================================================================

  # Friendly output column name -> raw ED2 variable name (yearly output has
  # NO resolution prefix at all, unlike monthly/daily/hourly - confirmed by
  # direct inspection: zero YMEAN_-prefixed variables exist).
  YEARLY_ECOSYSTEM_VAR_MAP <- c(
    AGB_total = "TOTAL_AGB", AGB_growth = "TOTAL_AGB_GROWTH",
    AGB_mort = "TOTAL_AGB_MORT", AGB_recruit = "TOTAL_AGB_RECRUIT",
    BasalArea_total = "TOTAL_BASAL_AREA", BasalArea_growth = "TOTAL_BASAL_AREA_GROWTH",
    BasalArea_mort = "TOTAL_BASAL_AREA_MORT", BasalArea_recruit = "TOTAL_BASAL_AREA_RECRUIT"
  )
  YEARLY_SIZECLASS_RAW_NAME <- c(
    AGB_growth = "AGB_GROWTH", AGB_mort = "AGB_MORT", AGB_cut = "AGB_CUT",
    BasalArea_growth = "BASAL_AREA_GROWTH", BasalArea_mort = "BASAL_AREA_MORT", BasalArea_cut = "BASAL_AREA_CUT"
  )
  YEARLY_ALL_SIZECLASS_VARS <- names(YEARLY_SIZECLASS_RAW_NAME)

  ecosystem_vars <- if (nzchar(variables_flag)) {
    requested <- trimws(strsplit(variables_flag, ",")[[1]])
    invalid <- setdiff(requested, names(YEARLY_ECOSYSTEM_VAR_MAP))
    if (length(invalid) > 0) {
      stop("--variables at --resolution=yearly must be a subset of: ",
           paste(names(YEARLY_ECOSYSTEM_VAR_MAP), collapse = ", "),
           " - got invalid: ", paste(invalid, collapse = ", "))
    }
    YEARLY_ECOSYSTEM_VAR_MAP[requested]
  } else {
    YEARLY_ECOSYSTEM_VAR_MAP
  }
  sizeclass_vars <- if (nzchar(sizeclass_variables_flag)) {
    req <- trimws(strsplit(sizeclass_variables_flag, ",")[[1]])
    invalid <- setdiff(req, YEARLY_ALL_SIZECLASS_VARS)
    if (length(invalid) > 0) {
      stop("--sizeclass_variables at --resolution=yearly must be a subset of: ",
           paste(YEARLY_ALL_SIZECLASS_VARS, collapse = ", "),
           " - got invalid: ", paste(invalid, collapse = ", "))
    }
    req
  } else {
    YEARLY_ALL_SIZECLASS_VARS
  }
  cat("Ecosystem-scale (demographic) variables:", paste(names(ecosystem_vars), collapse = ", "), "\n")
  cat("Size-class (demographic) variables:", paste(sizeclass_vars, collapse = ", "), "\n")

  # Raw (unprefixed) scalar read - yearly has no per-resolution prefix.
  get_raw_scalar <- function(f, nms, raw_name) {
    if (!(raw_name %in% nms)) return(NA_real_)
    as.numeric(f[[raw_name]]$read())
  }

  # Raw (unprefixed) PFT x size-class array (17 x 11), same shape/orientation
  # as get_pft_sizeclass_array() below but without any prefix fallback.
  get_raw_sizeclass_array <- function(f, nms, raw_name) {
    if (!(raw_name %in% nms)) {
      return(data.table(pft = integer(0), size_class_id = integer(0), value = numeric(0)))
    }
    arr <- f[[raw_name]]$read() # dims: (17, 11)
  ...
  demographic_sizeclass_rows <- list()
  base_grid <- CJ(pft = BCI_PFTS, size_class_id = seq_len(N_DBH))

  for (path in files) {
    fd <- parse_file_date(basename(path))
    datetime <- as.POSIXct(sprintf("%04d-%02d-%02d 00:00:00", fd$year, fd$month, fd$day), tz = "UTC")

    f <- H5File$new(path, mode = "r")
    nms <- f$ls()$name

    row_dt <- data.table(datetime = datetime, year = fd$year)
    for (i in seq_along(ecosystem_vars)) {
      row_dt[[names(ecosystem_vars)[i]]] <- get_raw_scalar(f, nms, ecosystem_vars[[i]])
    }
    demographic_rows[[length(demographic_rows) + 1]] <- row_dt

    merged <- copy(base_grid)
    for (v in sizeclass_vars) {
      tab <- get_raw_sizeclass_array(f, nms, YEARLY_SIZECLASS_RAW_NAME[[v]])
      setnames(tab, "value", v)
      merged <- merge(merged, tab, by = c("pft", "size_class_id"), all.x = TRUE)
    }
    merged[, `:=`(datetime = datetime, year = fd$year, size_class = size_class_labels[size_class_id])]
    demographic_sizeclass_rows[[length(demographic_sizeclass_rows) + 1]] <- merged

    f$close_all()
  }

  demographic_dt <- rbindlist(demographic_rows)
  setorder(demographic_dt, datetime)

  demographic_sizeclass_dt <- rbindlist(demographic_sizeclass_rows, fill = TRUE)
  setcolorder(demographic_sizeclass_dt, c("datetime", "year", "pft", "size_class_id", "size_class"))
  setorder(demographic_sizeclass_dt, datetime, pft, size_class_id)
  for (col in sizeclass_vars) demographic_sizeclass_dt[is.na(get(col)), (col) := 0]

  cat("\nUnits are ED2's native output units - AGB totals/rates are kgC/m2\n")
  cat("(rates are per year), basal area totals/rates are m2/m2 per year.\n")
  cat("Cross-check against ED2's documentation before using these in a paper.\n\n")

  out_eco_csv <- file.path(outdir, "demographic_output_yearly.csv")
  out_eco_rds <- file.path(outdir, "demographic_output_yearly.rds")
  fwrite(demographic_dt, out_eco_csv)
  saveRDS(demographic_dt, out_eco_rds)
  cat("Wrote", out_eco_csv, "\n")
  cat("Wrote", out_eco_rds, "(for plot_bci_output.R)\n\n")

  out_sc_csv <- file.path(outdir, "demographic_sizeclass_output_yearly.csv")
  out_sc_rds <- file.path(outdir, "demographic_sizeclass_output_yearly.rds")
  fwrite(demographic_sizeclass_dt, out_sc_csv)
  saveRDS(demographic_sizeclass_dt, out_sc_rds)
  cat("Wrote", out_sc_csv, "\n")
  cat("Wrote", out_sc_rds, "(for plot_bci_output.R)\n\n")

  cat("--- Preview (ecosystem-scale demographic totals) ---\n")
  print(demographic_dt)
} else {
  # ===========================================================================
  # MONTHLY / DAILY / HOURLY (flux/state time-series) extraction
  # ===========================================================================
  ...
  for (path in files) {
    ...
    base_datetime <- as.POSIXct(sprintf("%04d-%02d-%02d 00:00:00", year, month, day), tz = "UTC")
